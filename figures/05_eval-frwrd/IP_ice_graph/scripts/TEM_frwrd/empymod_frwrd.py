...
class empymod_frwrd(object):
    ...
    def calc_response(self, model, unit='res (ohmm)', scale='lin',
        ...
        if show_em_trafos:
            ...
            ax[1].loglog(self._fourier_trafo['freq'], np.real(self._EM_to_dBdt), '.r-', label='before cutoff-filter')
            ax[1].loglog(self._fourier_trafo['freq'], np.real(self._EM_after_bw), '+k', label='after cutoff-filter')

            ax[1].set_ylabel("fd-EM' (V/m²)")
            ax[1].set_title('real part of FD response')
            ax[1].legend()
            
            ax[2].loglog(self._fourier_trafo['freq'], abs(np.imag(self._EM_to_dBdt)), '.r-')
            ax[2].loglog(self._fourier_trafo['freq'], abs(np.imag(self._EM_after_bw)), '+k')

            ax[2].set_ylabel('fd-EM" (V/m²)')
            ax[2].set_title('imag part of FD response')
            
            ax[3].loglog(self._fourier_trafo['freq'], np.abs(self._EM_to_dBdt), '.r-')
            ax[3].loglog(self._fourier_trafo['freq'], np.abs(self._EM_after_bw), '+k')

            ax[3].set_ylabel("|fd-EM| (V/m²)")
            ax[3].set_title('magnitude of FD response')
            
            ax[4].loglog(self._fourier_trafo['freq'], abs(np.angle(self._EM_to_dBdt)), '.r-')
            ax[4].loglog(self._fourier_trafo['freq'], abs(np.angle(self._EM_after_bw)), '+k')

            ax[4].set_ylabel("\phi (rad)")
            ax[4].set_title('phase of FD response')
            ...
            f2plot = f'{cutofffreq:.1e}' if cutofffreq is not None else 'None'
            suptitle = (f'{self.setup_device["txloop"]:.2f} m loop, bw-filter cut-off freq: {f2plot} Hz' + 
