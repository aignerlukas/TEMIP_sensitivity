...
class Sounding():
    ...
    def plot_dBzdt(self, which='observed', ax=None,
                   xlimits=(2e-6, 1e-3), ylimits=(2e-10, 1e-1), sub0col='k',
        ...
        **kwargs : TYPE
        ...
        ax.loglog(self.time, abs(signal), marker='d', ls=':', 
                  lw=1, ms=5, **kwargs)
        ax.loglog(time_sub0, abs(sgnl_sub0), marker='s',
    ...
    def plot_rhoa(self, which='observed', ax=None, log_rhoa=False,
                  xlimits=(2e-6, 1e-3), ylimits=(1e0, 1e4), sub0col='k',
                  save_fid=None, dpi=150,
                   **kwargs):
        """
        ...
        **kwargs : TYPE
        ...
        ax.semilogx(self.time, abs(rhoa), marker='d', ls=':', 
                    lw=1, ms=5, **kwargs)
        ax.semilogx(time_sub0, abs(rhoa_sub0), marker='s',
                    markerfacecolor='none', markersize=4.5,
                    markeredgewidth=1, markeredgecolor=sub0col)
        ax.set_xlabel(r'time (s)')
