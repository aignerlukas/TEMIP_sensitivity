...
class simpeg_frwrd(object):
    ...
    def create_props_temfast(self, ramp_data='donauinsel', show_rampInt=True):
        """
        ...
        current = self.setup_device["current_inj"]
        currentkey = self.setup_device["currentkey"]
        ...
        if not currentkey in available_currents:
        ...
        rampdata = pd.read_csv(path2csv + f'TEMfast_rampOFF_{int(currentkey)}A.csv',
                               delimiter=',', skiprows=1)
    ...
    def infer_layer2mesh(self, model, show_mesh=False):
        ...
        if show_mesh:  # plotting part
            cm_discrete = cm.get_cmap('viridis', 8)
            ...
            axMdl.set_xlim((-20, 20))
            axMdl.set_ylim((-self.coredepth, 5))
