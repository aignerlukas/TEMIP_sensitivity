...
savepath_result = main + f'03-inv_results/fd-loopcomp-12-50/{version_inv}/'
savepath_dgsa = f'../../04_DGSA-figures/noIP/results_dgsa/fld-12-50/{version_dgsa}/'
...
# ax_fit = fig_result.add_subplot(gs[0, 0:3])
# ax_rho = fig_result.add_subplot(gs[0, 3:5])
# ax_fit1 = fig_result.add_subplot(gs[1, 0:3])
# ax_rho1 = fig_result.add_subplot(gs[1, 3:5])

# ax_thk = fig_result.add_subplot(gs[0, 5:6])
# ax_res = fig_result.add_subplot(gs[0, 6:7])
# ax_thk1 = fig_result.add_subplot(gs[1, 5:6])
# ax_res1 = fig_result.add_subplot(gs[1, 6:7])
...
if show_results:
    ...
    snd12 = survey.soundings[survey.sounding_names[1]]
    ...
    ax[0, 1].axhline(y=1.6, ls=':', color='chocolate', label='interface 1')
    ax[0, 1].axhline(y=7.5, ls=':', color='saddlebrown', label='interface 2')
    ...
    snd50 = survey.soundings[survey.sounding_names[0]]
...
for jdx, fid in enumerate(fids):
    ...
    for kdx, name in enumerate(param_names_long):
        ...
        if sens_logscale_x == True:
            ...
            # axes_dgsa[jdx, kdx].xaxis.set_minor_formatter(FormatStrFormatter("%.0f"))
            # axes_dgsa[jdx, kdx].xaxis.set_major_formatter(FormatStrFormatter("%.0f"))
    
            # axes_dgsa[jdx, kdx].xaxis.set_minor_locator(plt.MaxNLocator(n_ticks))
...
if save_fig:
    fig_result.savefig(savepath + f'fd-inv-dgsa_{version_vis}.png', dpi=300)
