...
main = './'
savepath = main + f'40_vis/{plot_typ}_{case}/inv-{version_inv}_dgsa-{version_dgsa}/{version_vis}/'
if not os.path.exists(savepath):
    os.makedirs(savepath)

savepath_data = main + f'30_inv-results/{case}_{inv_typ}/{version_inv}/'
# if not os.path.exists(savepath_data):
#     os.makedirs(savepath_data)

savepath_dgsa = ('../../04_DGSA-figures/TEMIP/results_dgsa/ice-50-03lay' +
                 f'/{version_dgsa}/')
...
fig_result = plt.figure(figsize=(7, 7), constrained_layout=False)
gs = fig_result.add_gridspec(2, 8)
...
axes_fit = np.array([ax_fit])
axes_mdl = np.array([[ax_rho, ax_mpa, ax_tau, ax_c]])
axes_sens = np.array([ax_sens])


ip_effect_names = ['T00']
ip_effect_types = ['$\ominus$IP']
...
for j, tau_val in enumerate(tau_vals):
    ...
    fid = f'sens_{cluster}clustr.csv'
