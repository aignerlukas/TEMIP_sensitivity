...
# read results  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
line_id = '2'
path2hb = '../../../03_fielddata-inv/Ice/'
name_file_raw = f'sb-line-0{line_id}'
name_file_result = f'sb-line-0{line_id}'
version = 'cs12_tr12-120us'  # v01_tr20-280us
time_range = np.r_[12, 120] * 1e-6
invrun = '001'
...
# results
result_path = path2hb + f'30_TEM-results/sb-l{line_id}_blk-6lay-mpa/{name_file_result}/{version}/'
...
priorTEM[5, 4:6] = np.r_[0.01, 0.011]  # replace same prior min and max
...
cstr_phim = np.r_[1, 0, 0, 0, 0, 1]
cstr_taup = np.r_[1, 0, 0, 0, 0, 1]
cstr_cs = np.r_[1, 0, 0, 0, 0, 1]
...
params_per_col=np.r_[5, 6, 4, 4, 4]
param_names_long = ['thickness (thk)', r'DC-resistivity ($\rho$)',
                    'max. phase angle ($\phi_{max}$)', 'relaxation time ($\\tau_{\phi}$)', 'dispersion coefficient ($c$)']
        ...
        for jdx, sens_i in enumerate(sens_s):
            ...
            for kdx, name in enumerate(param_names_long):
                ...
                axes[jdx, kdx].grid(which='major', axis='x', visible=True,
                                color='white', linestyle='--', linewidth=1.5)
                axes[0, kdx].set_xlabel('Sensitivity ()')
