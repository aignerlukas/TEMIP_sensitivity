...
class empymod_frwrd(object):
    ...
    def __init__(self, setup_device, setup_solver,
                 time_range=None, device='TEM-FAST',
        ...
        device : string, optional
            Name of tem device. The default is 'TEM-FAST'.
    ...
    def create_props_temfast(self, show_rampInt=False):
        """
        This method creates the device properties of the TEM-FAST device.
        Necessary to calculate the forward solution. It sets the class 
        attributes times_rx and properties_snd according to the selected 
        device measurement parameters given in setup_device.
        ...
        if not time_key in available_timekeys:
            message = ("You chose a time key that is not available for the TEM-FAST instrument." +
                       "Please select a time key between 1 and 9." +
                       "Creation of properties not succesful!!!")
            raise ValueError(message)

        if not current in available_currents:
            message = ("You chose a current that is not available for the TEM-FAST instrument." +
    ...
    def calc_response(self, model, mdl_unit='res (ohmm)', mdl_scale='lin',
        ...
        Parameters
        ----------
        model : np.ndarray (n x 2)
            thickness and resistivities of the resistivity model
            (see ``empymod.model.bipole`` for more info.)
        mdl_unit : string
            unit of the input model parameter, conductivity or resistivity
            in the array: model[:, 1] -> will be recalculated to resistivity
        mdl_scale : string
            scaling of the input resistivity (model[:,1]), 'lin' or 'log10'
        resp_trafo : string
        ...
        self.response : np.ndarray
            ...
            or apparent resistivity (Ohmm) if return_rhoa == True
